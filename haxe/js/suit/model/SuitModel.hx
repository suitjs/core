...
class SuitModel
{
	...
	{		
		var v : View = Std.is(p_target, String) ? Suit.view.get(p_target) : (cast p_target);
		var res : Dynamic = { };
		Suit.view.traverse(v,cast function(e:DOMElement):Void
		{				
			var it : View = cast e;
			...
			if (!has_children)
			{					
				var path : String = Suit.view.path(it, v);
