...
var js_suit_Suit = function() { };
js_suit_Suit.__name__ = true;
js_suit_Suit.init = function() {
	js_suit_Suit.model = new js_suit_model_SuitModel();
	js_suit_Suit.view = new js_suit_view_SuitView();
	js_suit_Suit.controller = new js_suit_controller_SuitController();
	js_suit_Suit.request = new js_suit_SuitRequest();
	window.addEventListener("load",function(e) {
		haxe_Timer.delay((function($this) {
			var $r;
			var delayed_component_cb = function() {
				window.dispatchEvent(new Event("component"));
				js_suit_Suit.view.parse();
			};
...
var js_suit_SuitRequest = function() {
};
js_suit_SuitRequest.__name__ = true;
js_suit_SuitRequest.prototype = {
	...
	,__class__: js_suit_SuitRequest
};
var js_suit_controller_Controller = function() {
	this.enabled = true;
	this.allow = ["click","input","change"];
};
js_suit_controller_Controller.__name__ = true;
js_suit_controller_Controller.prototype = {
	on: function(p_event) {
	}
	,__class__: js_suit_controller_Controller
};
var js_suit_controller_SuitController = function() {
	this.list = [];
};
js_suit_controller_SuitController.__name__ = true;
js_suit_controller_SuitController.prototype = {
	add: function(p_controller,p_target) {
		...
		var v;
		if(typeof(p_target) == "string") v = js_suit_Suit.view.get(p_target); else v = p_target;
		...
		if(p_controller.handler == null) {
			var controller_handler = function(e) {
				...
				cev.type = e.type;
				cev.src = e;
				if(js_Boot.__instanceof(e.target,Element)) cev.view = js_suit_Suit.view.path(e.target,v.parentElement); else cev.view = "";
	...
	,__class__: js_suit_controller_SuitController
};
var js_suit_model_SuitModel = function() {
};
js_suit_model_SuitModel.__name__ = true;
js_suit_model_SuitModel.prototype = {
	data: function(p_target,p_value) {
		var _g2 = this;
		var v;
		if(typeof(p_target) == "string") v = js_suit_Suit.view.get(p_target); else v = p_target;
		var res = { };
		js_suit_Suit.view.traverse(v,function(e) {
			var it = e;
			if(js_suit_Suit.view.name(it) == "") return;
			var has_children = true;
			if(it.children.length <= 0) has_children = false;
			if(it.nodeName.toLowerCase() == "select") has_children = false;
			if(!has_children) {
				var path = js_suit_Suit.view.path(it,v);
				var tks = path.split(".");
				var d = res;
				var dv = p_value;
				var _g1 = 0;
...
js_suit_view_SuitView.prototype = {
		...
		while(l.length > 0) {
			...
			this.traverse(it,(function($this) {
				...
				var get_traverse_cb = (function(f,n) {
					return function(e) {
						var v = e;
						if(n[0] == js_suit_Suit.view.name(v)) {
							f[0] = true;
							it = e;
							return false;
						}
	...
	,__class__: js_suit_view_SuitView
};
var $_, $fid = 0;
